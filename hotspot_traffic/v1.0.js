...
(async () => {
    ...
    const fetchManifest = async (mode) => {
        ...
        try {
            if (mode === 'init') {
                url = GH_VERSION_BASE + 'init_version.json';
            } else {
                if (!state._deviceVersion) return;
                url = GH_VERSION_BASE + 'update_v' + state._deviceVersion + '.json';
            }
            const r = await run(`curl -sL --connect-timeout 8 --max-time 15 ${sq(url)}`, 20000);
            text = String(r?.content || '').trim();
            if (!text) return;
            const j = JSON.parse(text);
            if (!j.version || !j.traffic || !j.diag) return;
            _manifest = { version: j.version, trafficUrl: j.traffic, diagUrl: j.diag, jsUrl: j.js || '' };
    ...
    const updatePluginJs = async (jsUrl) => {
        ...
        const r = await run(`cat ${sq(tmpJs)}; rm -f ${sq(tmpJs)}`, 10000);
        const newJs = String(r?.content || '');
        if (!newJs || newJs.length < 200) throw new Error('JS内容无效(长度=' + newJs.length + ')');
