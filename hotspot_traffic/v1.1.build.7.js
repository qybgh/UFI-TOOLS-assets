...
(async () => {
    ...
    const policyRemove = async (mac) => {
        await run(`[ -s ${sq(POLICY_FILE)} ] || printf '{}' > ${sq(POLICY_FILE)}`);
        const r = await run(`timeout 2s ${sq(JQ)} -c --arg m ${sq(mac)} 'del(.[$m])' ${sq(POLICY_FILE)} > ${sq(POLICY_FILE)}.tmp && mv ${sq(POLICY_FILE)}.tmp ${sq(POLICY_FILE)} && touch ${sq(POLICY_TRIGGER)} && echo __OK__ || { rm -f ${sq(POLICY_FILE)}.tmp; echo __FAIL__; }`);
        return { success: (r?.content || '').includes('__OK__'), content: r?.content };
    };
...
`, 5000);
    ...
    const probeIptables = async () => {
        const result = await run(`iptables -w 5 -L FORWARD -n 2>/dev/null && echo __OK__ || echo __FAIL__`, 5000);
...
pkill -f ${sq(SH_FILE)} 2>/dev/null
_tp=$(awk '{print}' ${sq(DATA_DIR + '/.tcpdump_pid')} 2>/dev/null)
[ -n "$_tp" ] && kill "$_tp" 2>/dev/null
sleep 1
[ -n "$_p" ] && kill -9 "$_p" 2>/dev/null
pkill -9 -f ${sq(SH_FILE)} 2>/dev/null
[ -n "$_tp" ] && kill -9 "$_tp" 2>/dev/null
rm -f ${sq(PID_FILE)} ${sq(DATA_DIR + '/.tcpdump_pid')}`.trim();

    const START_PROC = `cp ${sq(TRAFFIC_BIN_FILE)} ${TRAFFIC_PROC} && chmod 755 ${TRAFFIC_PROC} && nohup ${TRAFFIC_PROC} >/dev/null 2>&1 &`.trim();

    const IPT_CLEANUP = `iptables -w 5 -t mangle -D FORWARD -j HT_DEV_TX 2>/dev/null
iptables -w 5 -t mangle -F HT_DEV_TX 2>/dev/null
iptables -w 5 -t mangle -X HT_DEV_TX 2>/dev/null
iptables -w 5 -t mangle -D FORWARD -j HT_DEV_RX 2>/dev/null
iptables -w 5 -t mangle -F HT_DEV_RX 2>/dev/null
iptables -w 5 -t mangle -X HT_DEV_RX 2>/dev/null
ip6tables -w 5 -t mangle -D FORWARD -j HT_DEV6_TX 2>/dev/null
ip6tables -w 5 -t mangle -F HT_DEV6_TX 2>/dev/null
ip6tables -w 5 -t mangle -X HT_DEV6_TX 2>/dev/null
ip6tables -w 5 -t mangle -D FORWARD -j HT_DEV6_RX 2>/dev/null
ip6tables -w 5 -t mangle -F HT_DEV6_RX 2>/dev/null
ip6tables -w 5 -t mangle -X HT_DEV6_RX 2>/dev/null
iptables -w 5 -t filter -D FORWARD -j HT_BL 2>/dev/null
iptables -w 5 -t filter -F HT_BL 2>/dev/null
iptables -w 5 -t filter -X HT_BL 2>/dev/null
iptables -w 5 -t mangle -D FORWARD -j HT_THROTTLE 2>/dev/null
iptables -w 5 -t mangle -F HT_THROTTLE 2>/dev/null
iptables -w 5 -t mangle -X HT_THROTTLE 2>/dev/null
ip6tables -w 5 -t filter -D FORWARD -j HT_BL6 2>/dev/null
ip6tables -w 5 -t filter -F HT_BL6 2>/dev/null
ip6tables -w 5 -t filter -X HT_BL6 2>/dev/null
ip6tables -w 5 -t mangle -D FORWARD -j HT_THROTTLE6 2>/dev/null
ip6tables -w 5 -t mangle -F HT_THROTTLE6 2>/dev/null
ip6tables -w 5 -t mangle -X HT_THROTTLE6 2>/dev/null`;
...
${KILL_PROCS}
${IPT_CLEANUP}
rm -f ${sq(TRAFFIC_BIN_FILE)} ${sq(DIAG_BIN_FILE)} ${sq(SH_FILE)} ${sq(DIAG_SH_FILE)} ${sq(LOG_FILE)} ${TRAFFIC_PROC} ${DIAG_PROC} ${sq(WEBHOOK_FILE)}
rm -rf ${sq(DATA_DIR)}
`, 10000);
    ...
    const ensureStyle = () => {
      ...
      #${MODAL} .ht-edit-mini{background:rgba(102,126,234,.15);border:none;cursor:pointer;opacity:1;font-size:.6rem;padding:3px 6px;color:#a5b4fc;line-height:1;flex-shrink:0;border-radius:4px;transition:background .15s,color .15s;}
    ...
    const renderDeviceRow = (device, index) => {
        ...
        <td>
          <div class="ht-td-name">
            ...
            <button class="ht-edit-mini" data-edit-mac="${safeMac}" title="自定义名称">✎</button>
          </div>
...
`);
    ...
    const handleUpdateClick = async () => {
        ...
        if (_verTapCount === 1) return createToast('连续点击3次检查更新', 'blue', 1500);
    ...
    const showHelp = () => {
        ...
        toastEl.querySelector('#ht_help_qq').onclick = () => groupTip('群号已复制', 'green');
